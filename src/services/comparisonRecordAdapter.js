...
const toStoredModel = (result) => ({
  modelId: result.model?.modelId || null,
  name: result.model?.name || '未命名模型',
  format: result.model?.fileFormat || 'UNKNOWN',
  sourceType: result.model?.sourceType || 'unknown',
})

export function createPkStorageRecord(comparison) {
  if (!comparison || comparison.metadata?.source !== 'local_records') {
    throw new Error('only_local_record_comparisons_can_be_saved')
  }
  return {
    id: comparison.comparisonId,
    type: 'pk',
    modelA: toStoredModel(comparison.modelA),
    modelB: toStoredModel(comparison.modelB),
    scoresA: comparison.modelA.dimensionScores,
    scoresB: comparison.modelB.dimensionScores,
    totalScoreA: comparison.modelA.overallScore,
    totalScoreB: comparison.modelB.overallScore,
    gradeA: comparison.modelA.grade,
    gradeB: comparison.modelB.grade,
    winner: comparison.winners.overallWinner,
    comment: comparison.recommendation.rationale,
    rubricVersion: comparison.evaluationVersion,
    deliveryStatus: comparison.delivery,
